import {interpret,ModelError,type ModelConfig} from '../../../factory/app/model-command';
...
const json=(body:unknown,status=200)=>new Response(JSON.stringify(body),{status,headers:{'Content-Type':'application/json; charset=utf-8','Cache-Control':'no-store','X-Factory-API':'edgeone-1'}});
...
export async function onRequestPost({request,env}:{request:Request;env:Record<string,unknown>}){
 ...
 try{return json(await interpret(input,config(env)));}catch(e){return json({error:e instanceof ModelError?e.message:'服务暂时不可用，未执行任何动作。'},e instanceof ModelError?e.status:500);}
}
