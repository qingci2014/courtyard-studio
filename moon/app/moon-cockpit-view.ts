...
export class CockpitView{
 ...
 const sun=new T.DirectionalLight(0xeaf1ff,2.2);sun.position.set(-8,12,9);sun.castShadow=true;Object.assign(sun.shadow.camera,{left:-3.5,right:3.5,top:3.5,bottom:-3.5,near:.1,far:40});sun.shadow.mapSize.set(2048,2048);sun.shadow.bias=-.00008;sun.shadow.normalBias=.006;this.scene.add(sun);
 RectAreaLightUniformsLib.init();
 const area=(color:number,intensity:number,w:number,h:number,position:T.Vector3,target:T.Vector3)=>{const l=new T.RectAreaLight(color,intensity,w,h);l.position.copy(position);l.lookAt(target);this.scene.add(l);};
 area(0xffdfb6,18,.18,2.5,new T.Vector3(-1.39,2.43,-.25),new T.Vector3(0,.7,0));
 area(0xb4d3f4,12,.18,2.5,new T.Vector3(1.39,2.43,-.25),new T.Vector3(0,.7,0));
 area(0x95d5f4,3.2,1.05,.36,new T.Vector3(0,1.12,1.27),new T.Vector3(0,1.05,-.6));
 area(0xffe4cd,6,.8,.1,new T.Vector3(0,2.2,-1.7),new T.Vector3(0,1,0));
 ...
 this.ao=new GTAOPass(this.scene,this.camera);this.ao.blendIntensity=.85;this.ao.updateGtaoMaterial({radius:.23,thickness:.14,distanceFallOff:.65,scale:1,samples:12,screenSpaceRadius:false});this.ao.updatePdMaterial({radius:5,samples:12});this.ao.setSceneClipBox(new T.Box3(new T.Vector3(-2.15,-.2,-2.1),new T.Vector3(2.15,2.75,2.15)));
 // Transparent glazing and printed legends must not occlude the normal/depth pass.
 const renderAO=this.ao.render.bind(this.ao);this.ao.render=(renderer,write,read,delta,mask)=>{const visible=this.opticalSurfaces.map(o=>o.visible);this.opticalSurfaces.forEach(o=>{o.visible=false;});try{renderAO(renderer,write,read,delta,mask);}finally{this.opticalSurfaces.forEach((o,i)=>{o.visible=visible[i];});}};
 this.composer.addPass(this.ao);this.bloom=new UnrealBloomPass(new T.Vector2(1,1),.12,.25,1.3);this.composer.addPass(this.bloom);this.output=new OutputPass();this.composer.addPass(this.output);
 this.controls.addEventListener('change',()=>{this.dirty=true;});
 const resize=()=>{const w=Math.max(1,host.clientWidth),h=Math.max(1,host.clientHeight);this.renderer.setSize(w,h);this.composer.setSize(w,h);this.camera.aspect=w/h;this.camera.updateProjectionMatrix();this.dirty=true;};this.observer=new ResizeObserver(resize);this.observer.observe(host);resize();
 ...
 dispose(){if(this.dead)return;this.dead=true;this.signal.abort();this.observer.disconnect();this.controls.dispose();this.renderer.setAnimationLoop(null);this.ao.dispose();this.bloom.dispose();this.output.dispose();this.composer.dispose();this.release(this.scene);this.scene.environment?.dispose();this.scene.traverse(o=>{if(o instanceof T.DirectionalLight||o instanceof T.PointLight)o.shadow.dispose();});this.renderer.dispose();this.renderer.domElement.remove();}
}
