...
export class HandInput {
 private model:HandLandmarker|null=null; private stream:MediaStream|null=null; private raf=0; private lastVideo=-1; private lastInference=0; private lastSeen=0; private pinch=false; private missing=false; private active=false; private candidate=false; private candidateSince=0;
 ...
 async start(){
  ...
  try {
   ...
   this.active=true;this.lastSeen=performance.now();this.lastVideo=-1;this.lastInference=0;this.pinch=false;this.candidate=false;this.missing=false;
 ...
 private loop=(now:number)=>{if(!this.active)return;this.raf=requestAnimationFrame(this.loop);if(document.hidden||now-this.lastInference<65||this.video.readyState<2||this.video.currentTime===this.lastVideo)return;this.lastInference=now;this.lastVideo=this.video.currentTime;
  try{const result=this.model!.detectForVideo(this.video,now);const points=result.landmarks[0];this.draw(points);
   if(!points){this.hooks.frame(null);if(now-this.lastSeen>700&&!this.missing){this.missing=true;this.pinch=false;this.candidate=false;this.hooks.lost();}return;}
   ...
   const candidate=this.pinch?ratio<.55:ratio<.32;
   if(candidate!==this.candidate){this.candidate=candidate;this.candidateSince=now;}
   if(now-this.candidateSince>90)this.pinch=this.candidate;
   const cx=(wrist.x+middle.x+points[5]!.x+points[17]!.x)/4;
   const cy=(wrist.y+middle.y+points[5]!.y+points[17]!.y)/4;
   this.hooks.frame({x:((1-cx)-.5)*11,z:(cy-.5)*7,pinch:this.pinch});
